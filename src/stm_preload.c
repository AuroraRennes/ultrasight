...
/* __attribute__((constructor)) tells GCC/Clang to run this function automatically before main() */
...
void preload_stm_region(void) {
    ...
    void* mapped_region = mmap(NULL, STM_MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, STM_STIMULUS_BASE);
...
/* __attribute__((constructor)) tells GCC/Clang to run this function automatically after main() */
